...
export default function ProgramDetailClient() {
  ...
  const loadData = async () => {
    ...
    let completionByClientData: Record<string, number> = {}
    if (videoIdsInProgram.length > 0 && clientIds.length > 0) {
      ...
      for (const cid of clientIds) {
        completionByClientData[cid] = videoIdsInProgram.filter((vid) => set.has(`${cid}:${vid}`)).length
      }
    }

    setProgram(programData)
    setLessons(lessonsData || [])
    setCompletionByClient(completionByClientData)
  ...
  const handleAssignClients = async (clientIds: string[]) => {
    ...
    if (assignErr) {
      setAssignError('Could not add access. Please try again.')
      await loadData()
      return
    }
    ...
    <div className="space-y-8">
      ...
      <Card className="border-[var(--cp-border-subtle)] bg-[var(--cp-bg-surface)]">
        ...
        <CardContent className="space-y-4">
          {assignError && (
            <p className="text-sm text-[var(--cp-accent-danger)]">{assignError}</p>
          )}
          {allClients.length > 0 && (
            <div className="flex gap-2 flex-wrap items-end">
              <div className="min-w-[220px] flex-1">
                ...
                <p className="text-xs text-[var(--cp-text-muted)] mt-1">Select a client, then click Add access.</p>
              </div>
              <Button
                size="sm"
                onClick={() => {
                  if (assignClientId) {
                    handleAssignClients([assignClientId])
                  }
                }}
