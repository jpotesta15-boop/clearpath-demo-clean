...
export type ClientScheduleInitialData = {
  client: any
...
export function ClientScheduleContent({ initialData }: { initialData?: ClientScheduleInitialData | null }) {
  const [slots, setSlots] = useState<any[]>(initialData?.slots ?? [])
  const [sessions, setSessions] = useState<any[]>(initialData?.sessions ?? [])
  const [sessionRequests, setSessionRequests] = useState<any[]>(initialData?.sessionRequests ?? [])
  const [loading, setLoading] = useState(!initialData?.client)
  const [client, setClient] = useState<any>(initialData?.client ?? null)
  const [submittingRequestId, setSubmittingRequestId] = useState<string | null>(null)
  const [payError, setPayError] = useState<string | null>(null)
  const [decliningRequestId, setDecliningRequestId] = useState<string | null>(null)
  const [availabilityText, setAvailabilityText] = useState('')
  const [submittingAvailability, setSubmittingAvailability] = useState(false)
  const [coachTimezone, setCoachTimezone] = useState<string | null>(initialData?.coachTimezone ?? null)
  const [timeRequestText, setTimeRequestText] = useState('')
  const [submittingTimeRequest, setSubmittingTimeRequest] = useState(false)
  const [timeRequests, setTimeRequests] = useState<any[]>(initialData?.timeRequests ?? [])
  ...
  const handleCancelSession = async (session: { id: string; session_request_id?: string | null }) => {
